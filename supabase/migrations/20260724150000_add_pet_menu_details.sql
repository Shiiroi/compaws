-- 1. Add pet_menu_details jsonb column to places table
alter table places add column if not exists pet_menu_details jsonb default null;

-- 2. RPC function to update pet menu details for a place
create or replace function update_place_pet_menu(
  p_place_id uuid,
  p_pet_menu_details jsonb
)
returns boolean
language plpgsql
security definer
...
begin
  update places
  set pet_menu_details = p_pet_menu_details,
      updated_at = now()
  where id = p_place_id;
...
-- 3. Re-create get_places_in_bounds RPC returning pet_menu_details
