-- 1. Add operating_hours jsonb column to places table
alter table places add column if not exists operating_hours jsonb default null;

-- 2. Drop and recreate create_place_with_report RPC with p_operating_hours parameter
...
-- 3. Re-create get_places_in_bounds RPC returning operating_hours
...
-- 4. RPC function to update operating hours for a place
create or replace function update_place_operating_hours(
  p_place_id uuid,
...
begin
  update places
  set operating_hours = p_operating_hours,
      updated_at = now()
  where id = p_place_id;
